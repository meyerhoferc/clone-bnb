...
describe "traveler can make a review" do
  it "for each listing a traveler has stayed at" do
    ...
    # two = traveler.reservations.create!(start_date: "1/24/17", end_date: "1/30/17", listing_id: a_listing.id, status: "complete")
    #three = traveler.reservations.create!(start_date: "3/14/17", end_date: "3/16/17", listing_id: a_listing.id, status: "complete")
    #four = traveler.reservations.create!(start_date: "3/1/17", end_date: "3/2/17", listing_id: a_listing.id, status: "complete")
    ...
    expect(page).to have(a_listing.street_address)
    ...
    expect(current_path).to eq(new_listing_review(a_listing))
